...
namespace ByteTrackBase
{
    ...
    public sealed class ByteTracker
    {
        ...
        /// <param name="trackBuffer">跟踪缓冲区的大小 表示跟踪器在丢失目标后，仍然保留目标信息的时间窗口。这个值越大，跟踪器对目标的“记忆”时间越长，但可能会增加计算复杂度 建议范围：15-60帧。值越大，容忍遮挡/漏检时间越长，但可能残留"幽灵"轨迹。</param>
        /// <param name="trackThresh">跟踪阈值 用于决定一个检测结果是否足够可靠，可以被初始化为一个新的跟踪目标。通常是一个置信度阈值，只有置信度高于此阈值的检测结果才会被考虑  建议范围：0.4–0.7。值越高，第一轮关联的检测框越可靠，但可能丢失部分目标。</param>
        /// <param name="highThresh">高置信度阈值 用于区分高置信度和低置信度的检测结果。高置信度的检测结果通常会被优先处理，而低置信度的检测结果可能会被进一步验证或忽略  建议范围：0.05–0.3。值越低，越能挽回遮挡目标，但噪声风险增加。</param>
        /// <param name="matchThresh">匹配阈值 用于决定两个检测结果是否足够相似，可以被认为是同一个目标。通常是一个相似度阈值，只有相似度高于此阈值的检测结果才会被认为是同一个目标 建议范围：0.6–0.9。值越高，匹配要求越严格，ID切换少，但未匹配轨迹增多。</param>
