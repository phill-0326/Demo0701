...
namespace YoloOnnx
{
    public class Yolo_OnnxRuntime : IDisposable
    {
        ...
        bool isCuda = false;
        public bool IsCuda { get => IsCuda; }

        ModelInfo modelInfo;
        public ModelInfo ModelInfo { get => modelInfo; }

        public Yolo_OnnxRuntime()
        {

        }

        #region 加载Onnx模型
        public ModelInfo LoadOnnxModel(string path, bool isCuda, int cudaId = 0)
        {
            this.isCuda = isCuda;

            if (isCuda)
            {
                session = new InferenceSession(path, SessionOptions.MakeSessionOptionWithCudaProvider(cudaId));
            }
            else
            {
            ...
            //获取模型信息
            model = ModelProto.Parser.ParseFromFile(path);
            var infos = model.MetadataProps.ToList();
            ...
            this.modelType = (ModelType)Enum.Parse(typeof(ModelType), modelInfo.TaskName);
        ...
        public ModelInfo LoadOnnxModel(string path, bool isCuda, ModelType modelType, int cudaId = 0)
        {
            this.isCuda = isCuda;
        ...
        */
        private InferenceResult InferenceObb(Bitmap image, float confidence = 0.5f, float iou = 0.5f)
        {
            try
            {
                ...
                int targetWidth = session.InputMetadata["images"].Dimensions[3];
                int targetHeight = session.InputMetadata["images"].Dimensions[2];
                ...
                Parallel.For(0, outputTensor.Dimensions[1] - 5, j =>
                {
                    for (int i = 0; i < outputTensor.Dimensions[2]; i++)
                    {
                        float score = outputTensor[0, 4 + j, i];
                        if (score > confidence)
                        {
                            ...
                            float r = outputTensor[0, outputTensor.Dimensions[1] - 1, i];
                            ...
                            Boxes box = new Boxes()
                            {
                                Confidence = score,
                                LabelId = j,
                                LabelName = labelNames.Count > 0 ? labelNames[j] : "",
                                Angle = r,
                                X = (x - padX) / ratio,
                                Y = (y - padY) / ratio,
                                Width = w / ratio,
        ...
        private InferenceResult InferenceDetect(Bitmap image, float confidence = 0.5f, float iou = 0.5f)
        {
            try
            {
                ...
                //根据置信度筛选 目标检测:输出的模型格式为[1,84,8400]即代表1个batch 6则代表边界框0:X 1:Y 2:W 3:H 4-83:分类的Label置信度 8400则代表有输出8400个边界框 
                ConcurrentBag<Boxes> boxes = new ConcurrentBag<Boxes>();

                // 计算填充的像素数
                float padX = (targetWidth - ratio * ortImgWidth) / 2;
                float padY = (targetHeight - ratio * ortImgHeight) / 2;
                Parallel.For(0, outputTensor.Dimensions[1] - 4, j =>
                {
                    for (int i = 0; i < outputTensor.Dimensions[2]; i++)
                    {
                        float score = outputTensor[0, 4 + j, i];
        ...
        private InferenceResult InferenceClassify(Bitmap image, float confidence = 0.5f)
        {
            try
            {
                ...
                var maxElement = array.Select((maxConfidence, index) => new { MaxConfidence = maxConfidence, Index = index }).OrderByDescending(x => x.MaxConfidence).First();
                var selectArrar = array.Select((confidences, indexes) => new { Confidences = confidences, Indexes = indexes }).Where(x => x.Confidences >= confidence).ToArray();
        ...
        private InferenceResult InferenceSegment(Bitmap image, float confidence = 0.5f, float iou = 0.5f)
        {
            try
            {
                ...
                var bitmap = LetterboxResize(image, targetWidth, targetHeight, Color.FromArgb(114, 114, 114));
                //图片归一化操作
                //var tensor = NormalizeImage(bitmap, targetWidth, targetHeight);
                var tensor = NormalizeImage(bitmap);
                ...
                Tensor<float> maskTensor = results[1].AsTensor<float>();
                //获取标签
                Dictionary<int, string> labelNames = new Dictionary<int, string>();
                if (this.modelInfo != null)
                {
                ...
                float padX = (targetWidth - ratio * ortImgWidth) / 2;
                float padY = (targetHeight - ratio * ortImgHeight) / 2;
                Parallel.For(0, outputTensor.Dimensions[1] - 4 - maskTensor.Dimensions[1], j =>
                {
                    for (int i = 0; i < outputTensor.Dimensions[2]; i++)
                    {
                        float score = outputTensor[0, 4 + j, i];
                ...
                {
                    //计算mask权重值
                    var maskWeights = ExtractMaskWeights(outputTensor, item.BoxIndex, maskTensor.Dimensions[1], outputTensor.Dimensions[1] - maskTensor.Dimensions[1]);

                    //将按原图box值转成mask的尺寸并进行权重值计算得到mask(缩小遍历次数，每次只获取box范围内的mask值)
                    float x = (item.X * ratio + padX) / (targetWidth / maskTensor.Dimensions[3]);
                    float y = (item.Y * ratio + padY) / (targetHeight / maskTensor.Dimensions[2]);
                    float w = item.Width * ratio / (targetWidth / maskTensor.Dimensions[3]);
                    float h = item.Height * ratio / (targetHeight / maskTensor.Dimensions[2]);
                    int boxX = Math.Max(0, (int)(x - w / 2) - 1);
                    int boxY = Math.Max(0, (int)(y - h / 2) - 1);
                    int maxX = Math.Min(boxX + (int)w + 2, maskTensor.Dimensions[3]);
                    int maxY = Math.Min(boxY + (int)h + 2, maskTensor.Dimensions[2]);
                    float[,] mask = ProcessMask(maskTensor, maskWeights, boxX, boxY, maxX, maxY);

                    //将mask缩放至原图尺寸大小
                    item.Mask = ResizeMaskAfterLetterbox(mask, item, ortImgWidth, ortImgHeight);
                }
                time.Stop();
                postprocessTime = time.ElapsedMilliseconds;
        ...
        private InferenceResult InferencePose(Bitmap image, float confidence = 0.5f, float iou = 0.5f)
        {
            try
            {
                ...
                float padX = (targetWidth - ratio * ortImgWidth) / 2;
                float padY = (targetHeight - ratio * ortImgHeight) / 2;
                Parallel.For(0, outputTensor.Dimensions[2], i =>
                {
                    float score = outputTensor[0, 4, i];
                    if (score > confidence)
                    {
                        float x = outputTensor[0, 0, i];
                        float y = outputTensor[0, 1, i];
                        ...
                        {
                            int index = 5 + j * 3;
                            float keypointX = outputTensor[0, index, i];
                            float keypointY = outputTensor[0, index+1, i];
                            float keypointVisibility = outputTensor[0, index+2, i];
        ...
        /// <param name="inferTimeInfo">推理耗时统计输出</param>
        /// <param name="targetWidth">图片输入宽</param>
        /// <param name="targetHeight">图片输入高</param>
        /// <param name="confidence">置信度</param>
        /// <param name="iou">iou范围</param>
        /// <returns></returns>
        public InferenceResult[] Inference_Dyamics(List<Bitmap> images, out InferTimeInfo inferTimeInfo, int targetWidth = 640, int targetHeight = 640, float confidence = 0.5f, float iou = 0.5f)
        {
            lock (this)
            {
                ...
                switch (modelType)
                {
                    ...
                    case ModelType.classify:
                        return InferenceClassify_Dyamics(images, out inferTimeInfo, targetWidth, targetHeight, confidence);
                    case ModelType.segment:
        ...
        */
        private InferenceResult[] InferenceObb_Dyamics(List<Bitmap> images, out InferTimeInfo inferTimeInfo, int targetWidth = 640, int targetHeight = 640, float confidence = 0.5f, float iou = 0.5f)
        {
            try
            {
                ...
                //图片归一化操作
                var tensor = NormalizeImage_Dyamics(bitmap);
                ...
                for (int l = 0; l < images.Count; l++)
                {
                    ...
                    Parallel.For(0, outputTensor.Dimensions[1] - 5, j =>
                    {
                        for (int i = 0; i < outputTensor.Dimensions[2]; i++)
                        {
                            float score = outputTensor[l, 4 + j, i];
                            if (score > confidence)
                            {
                                float x = outputTensor[l, 0, i];
                                float y = outputTensor[l, 1, i];
                                float w = outputTensor[l, 2, i];
                                float h = outputTensor[l, 3, i];
                                float r = outputTensor[l, outputTensor.Dimensions[1] - 1, i];
         ...
         张量 output: float32[1, 1000] 的结构说明
            ...
            分类输出张量的处理步骤
                ...
                类别映射:
                    YOLOv8 分类模型通常有预定义的 1000 个类别映射表。通过索引，可以将预测的索引值转换为相应的类别名称。
         */
        private InferenceResult[] InferenceClassify_Dyamics(List<Bitmap> images, out InferTimeInfo inferTimeInfo, int targetWidth = 640, int targetHeight = 640, float confidence = 0.5f)
        {
            try
            {
                inferTimeInfo = new InferTimeInfo();
                ...
                //图片归一化操作
                var tensors = NormalizeImage_Dyamics(bitmap);

                //创建输入
                var inputData = new List<NamedOnnxValue>()
                {
                    NamedOnnxValue.CreateFromTensor<float>("images", tensors)
                };
                time.Stop();
                inferTimeInfo.PreprocessTime = time.ElapsedMilliseconds;
                ...
                for (int l = 0; l < images.Count; l++)
                {
                    ...
                    var array = outputTensor.ToArray();
                    float[] tempFloat = new float[outputTensor.Dimensions[1]];
                    int startIndex = l * outputTensor.Dimensions[1];
                    Array.Copy(array, startIndex, tempFloat, 0, outputTensor.Dimensions[1]);
        ...
        /// </summary>
        /// <param name="images">图片集合</param>
        /// <param name="targetWidth">输入图片宽</param>
        /// <param name="targetHeight">输入图片高</param>
        /// <param name="confidence">置信度</param>
        /// <param name="iou">iou范围</param>
        /// <returns></returns>
        private InferenceResult[] InferenceSegment_Dyamics(List<Bitmap> images, out InferTimeInfo inferTimeInfo, int targetWidth = 640, int targetHeight = 640, float confidence = 0.5f, float iou = 0.5f)
        {
            try
            {
                ...
                List<InferenceResult> InferenceResults = new List<InferenceResult>();
                //Parallel.For(0, bitmaps.Count, l =>
                for (int l = 0; l < images.Count; l++)
                {
                    ConcurrentBag<Boxes> boxes = new ConcurrentBag<Boxes>();
                    //Bitmap image = new Bitmap(images[l]);
                    Bitmap image = images[l];
                    //Bitmap image = bitmaps.ElementAt(l);
                    int ortImgWidth = image.Width;
                    int ortImgHeight = image.Height;
                    float ratio = Math.Min((float)targetWidth / ortImgWidth, (float)targetHeight / ortImgHeight);
                    ...
                    {
                        //计算mask权重值
                        var maskWeights = ExtractMaskWeights_Dyamics(outputTensor, l, item.BoxIndex, maskTensor.Dimensions[1], outputTensor.Dimensions[1] - maskTensor.Dimensions[1]);
                        ...
                        int maxX = Math.Min(boxX + (int)w + 2, maskTensor.Dimensions[3]);
                        int maxY = Math.Min(boxY + (int)h + 2, maskTensor.Dimensions[2]);
                        float[,] mask = ProcessMask_Dyamics(maskTensor, l, maskWeights, boxX, boxY, maxX, maxY);
        ...
        private InferenceResult[] InferencePose_Dyamics(List<Bitmap> images, out InferTimeInfo inferTimeInfo, int targetWidth = 640, int targetHeight = 640, float confidence = 0.5f, float iou = 0.5f)
        {
            try
            {
                ...
                for (int i = 0; i < images.Count; i++)
                {
                    ...
                    float padX = (targetWidth - ratio * ortImgWidth) / 2;
                    float padY = (targetHeight - ratio * ortImgHeight) / 2;
                    Parallel.For(0, outputTensor.Dimensions[2], k =>
                    {
                        float score = outputTensor[i, 4, k];
                        if (score > confidence)
                        {
                            float x = outputTensor[i, 0, k];
                            float y = outputTensor[i, 1, k];
                            float w = outputTensor[i, 2, k];
                            float h = outputTensor[i, 3, k];
                            float r = 0;
                            ...
                            {
                                int index = 5 + j * 3;
                                float keypointX = outputTensor[i, index, k];
                                float keypointY = outputTensor[i, index + 1, k];
                                float keypointVisibility = outputTensor[i, index + 2, k];
                                string keypointName = KeyPoint.KeypointNames[j];
                                keypoints[j] = new KeyPoint()
                                {
                                    X = (keypointX - padX) / ratio,
        ...
        /// <returns></returns>
        private float[,] ProcessMask(Tensor<float> maskTensor, float[] maskWeights, int X, int Y, int maxX, int maxY)
        {
            ...
            Parallel.For(Y, maxY, y =>
            {
                for (int x = X; x < maxX; x++)
                {
                    var value = 0f;
                    // 提取对应类别的掩码
                    for (int z = 0; z < maskTensor.Dimensions[1]; z++)
                    {
        ...
        /// <param name="maskWeightsOffset"></param>
        /// <returns></returns>
        private static float[] ExtractMaskWeights_Dyamics(Tensor<float> output, int index, int boxIndex, int maskChannelCount, int maskWeightsOffset)
        {
            var maskWeights = new float[maskChannelCount];
        ...
        private float[,] ProcessMask_Dyamics(Tensor<float> maskTensor, int index, float[] maskWeights, int X, int Y, int maxX, int maxY)
        {
            int h = maskTensor.Dimensions[2];
            int w = maskTensor.Dimensions[3];
            float[,] mask = new float[h, w];
        ...
        public void SaveMaskAsImage(float[,] mask, string filePath)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    // 将浮点值映射到颜色范围（例如：0.0f -> 黑色，1.0f -> 白色）
        ...
        public Tensor<float> NormalizeImage(Bitmap bitmap)
        {
        ...
        public void ExtractPixelsArgb(DenseTensor<float> tensor, Span<byte> data, int pixelCount)
        {
        ...
        public void ExtractPixelsRgb(DenseTensor<float> tensor, Span<byte> data, int pixelCount)
        {
            var spanR = tensor.Buffer.Span;
            var spanG = spanR.Slice(pixelCount);
            var spanB = spanG.Slice(pixelCount);
        ...
        #region 动态输入尺寸模型
        public Tensor<float> NormalizeImage_Dyamics(List<Bitmap> bitmaps)
        {
            if (bitmaps == null || bitmaps.Count == 0)
            {
                throw new ArgumentException("The bitmaps list cannot be null or empty.");
            }

            int batchSize = bitmaps.Count;
            int height = bitmaps[0].Height;
            int width = bitmaps[0].Width;
            var tensor = new DenseTensor<float>(new[] { batchSize, 3, height, width });
            ...
            Parallel.For(0, batchSize, i =>
            {
                ...
                if (bitmap.PixelFormat == PixelFormat.Format24bppRgb && bitmap.Width % 4 == 0)
                {
                    ...
                    ExtractPixelsRgb_Dyamics(tensor, data, pixelCount, i);
                }
                else
                {
                    // force convert to 32 bit PArgb
                    ...
                    ExtractPixelsArgb_Dyamics(tensor, data, pixelCount, i);
                }
        ...
        private void ExtractPixelsArgb_Dyamics(DenseTensor<float> tensor, Span<byte> data, int pixelCount, int batchIndex)
        {
            var spanR = tensor.Buffer.Span.Slice(batchIndex * 3 * pixelCount);
            var spanG = spanR.Slice(pixelCount);
            var spanB = spanG.Slice(pixelCount);
        ...
        private void ExtractPixelsRgb_Dyamics(DenseTensor<float> tensor, Span<byte> data, int pixelCount, int batchIndex)
        {
            var spanR = tensor.Buffer.Span.Slice(batchIndex * 3 * pixelCount);
            var spanG = spanR.Slice(pixelCount);
            var spanB = spanG.Slice(pixelCount);
